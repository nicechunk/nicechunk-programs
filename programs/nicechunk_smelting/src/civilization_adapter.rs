...
use crate::errors::{require_key_eq, NicechunkSmeltingError};
...
pub fn validate_rule_book_for_smelting_patch(
    data: &[u8],
    civilization_program: &Pubkey,
    smelting_program: &Pubkey,
    recipe_table: &Pubkey,
    patch_payload: &[u8],
    expected_status: u8,
) -> ProgramResult {
    ...
    if data.len() < RULE_BOOK_TARGET_PDA_OFFSET + 32 {
        return Err(NicechunkSmeltingError::InvalidCivilizationRule.into());
    }
    if data.get(RULE_BOOK_STATUS_OFFSET).copied() != Some(expected_status) {
        return Err(NicechunkSmeltingError::InvalidCivilizationRule.into());
    }
    if pubkey_at(data, RULE_BOOK_TARGET_PROGRAM_OFFSET)? != *smelting_program {
        return Err(NicechunkSmeltingError::CivilizationTargetMismatch.into());
    }
    if pubkey_at(data, RULE_BOOK_TARGET_PDA_OFFSET)? != *recipe_table {
        return Err(NicechunkSmeltingError::CivilizationTargetMismatch.into());
    }
    let patch_hash = hash(patch_payload);
    if &data[RULE_BOOK_PATCH_HASH_OFFSET..RULE_BOOK_PATCH_HASH_OFFSET + 32] != patch_hash.as_ref() {
        return Err(NicechunkSmeltingError::CivilizationPatchHashMismatch.into());
    }
    if civilization_program == smelting_program {
...
pub fn validate_tally_threshold(data: &[u8], rule_book: &Pubkey) -> ProgramResult {
    ...
    if pubkey_at(data, TALLY_RULE_BOOK_OFFSET)? != *rule_book {
        return Err(NicechunkSmeltingError::InvalidCivilizationTally.into());
    }
    if data.get(TALLY_THRESHOLD_MET_OFFSET).copied() != Some(1) {
        return Err(NicechunkSmeltingError::CivilizationThresholdNotMet.into());
    }
    Ok(())
}
...
pub fn validate_execution_receipt(data: &[u8], rule_book: &Pubkey) -> ProgramResult {
    ...
    if data.get(RECEIPT_EXECUTED_OFFSET).copied() != Some(1) {
        return Err(NicechunkSmeltingError::InvalidCivilizationReceipt.into());
    }
...
pub fn invoke_civilization_execute_receipt<'a>(
    ...
    let adapter_bump =
        validate_adapter_authority_pda(adapter_authority.key, smelting_program, rule_book.key)?;
    let ix = Instruction {
...
pub fn validate_adapter_authority_pda(
    ...
    smelting_program: &Pubkey,
    rule_book: &Pubkey,
) -> Result<u8, NicechunkSmeltingError> {
    let (expected, bump) = Pubkey::find_program_address(
        &[CIVILIZATION_ADAPTER_AUTHORITY_SEED, rule_book.as_ref()],
        smelting_program,
    );
    if &expected != adapter_authority {
        return Err(NicechunkSmeltingError::InvalidCivilizationProgram);
    }
    Ok(bump)
}
...
fn validate_header(data: &[u8], magic: &[u8; 8]) -> ProgramResult {
    if data.len() < 12 {
        return Err(NicechunkSmeltingError::InvalidCivilizationAccount.into());
    }
...
fn pubkey_at(data: &[u8], offset: usize) -> Result<Pubkey, NicechunkSmeltingError> {
    if offset + 32 > data.len() {
        return Err(NicechunkSmeltingError::InvalidCivilizationAccount);
    }
    Ok(Pubkey::new_from_array(
        data[offset..offset + 32]
            .try_into()
            .map_err(|_| NicechunkSmeltingError::InvalidCivilizationAccount)?,
    ))
}
...
mod tests {
    ...
    fn validates_target_and_patch_hash() {
        ...
        let smelting_program = Pubkey::new_unique();
        ...
        data[RULE_BOOK_TARGET_PROGRAM_OFFSET..RULE_BOOK_TARGET_PROGRAM_OFFSET + 32]
            .copy_from_slice(smelting_program.as_ref());
        data[RULE_BOOK_TARGET_PDA_OFFSET..RULE_BOOK_TARGET_PDA_OFFSET + 32]
            .copy_from_slice(recipe_table.as_ref());

        validate_rule_book_for_smelting_patch(
            &data,
        ...
        let err = validate_rule_book_for_smelting_patch(
            &data,
            &civilization_program,
            &smelting_program,
            &recipe_table,
        ...
        assert_eq!(
            ...
            solana_program::program_error::ProgramError::Custom(
                NicechunkSmeltingError::CivilizationPatchHashMismatch as u32,
            ),
        );
    }

    #[test]
    fn validates_tally_and_receipt_rule_binding() {
        let rule_book = Pubkey::new_unique();
        let mut tally = [0_u8; 128];
        tally[0..8].copy_from_slice(&CIVILIZATION_TALLY_MAGIC);
        tally[8..10].copy_from_slice(&CIVILIZATION_VERSION.to_le_bytes());
        tally[TALLY_THRESHOLD_MET_OFFSET] = 1;
        tally[TALLY_RULE_BOOK_OFFSET..TALLY_RULE_BOOK_OFFSET + 32]
            .copy_from_slice(rule_book.as_ref());
        validate_tally_threshold(&tally, &rule_book).unwrap();

        let mut receipt = [0_u8; 128];
        receipt[0..8].copy_from_slice(&CIVILIZATION_EXECUTION_RECEIPT_MAGIC);
        receipt[8..10].copy_from_slice(&CIVILIZATION_VERSION.to_le_bytes());
        receipt[RECEIPT_EXECUTED_OFFSET] = 1;
        receipt[RECEIPT_RULE_BOOK_OFFSET..RECEIPT_RULE_BOOK_OFFSET + 32]
            .copy_from_slice(rule_book.as_ref());
        validate_execution_receipt(&receipt, &rule_book).unwrap();
    }
}
